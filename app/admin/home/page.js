...
//                             <Inbox size={16} /> Queue
//                         </button>
//                         <button onClick={() => router.push('/admin/analytics')} className="w-full flex items-center gap-3 p-2.5 text-xs font-bold uppercase tracking-wider text-slate-500 hover:text-indigo-600 hover:bg-slate-50 transition-all rounded-sm">
...
const AdminHome = () => {
    ...
    // Your logic: changing this triggers the useEffect to re-fetch
    const [update, setUpdate] = React.useState(0);
    ...
    // --- 2. AUTH & FETCH LOGIC (Kept exactly as yours) ---
    const fetchAdminInfo = async () => {
        const userId = Cookies.get('userId');
    ...
    const getAllComplaints = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setComplaints(result.complaints);
                // Note: I removed updateComplaintCounts here because we can calculate stats on the fly in the child components
            }
        } catch (error) {
            console.error('Error fetching complaints:', error);
        }
    ...
    // --- 3. API UPDATE LOGIC ---
    // This function is passed to QueueBoard. 
    // It takes the modified complaint object, sends it to DB, then refreshes the page.
    const handleSaveChanges = async (updatedComplaint) => {
        ...
        try {
            ...
            if (response.ok) {
                // Your logic: Toggle update state to trigger useEffect refresh
                setUpdate((prev) => (prev === 0 ? 1 : 0));
            }
        } catch (error) {
            console.error('Error updating complaint:', error);
    ...
    React.useEffect(() => {
        ...
        getAllComplaints();
        getAllEmployees();
    }, [update]); // Re-runs when 'update' changes
