...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:8800/login/admin', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                Cookies.set('token', data.token, { path: '/' }); // Expires in 1 day
                Cookies.set('isLoggedIn', 'true', { path: '/' });
                Cookies.set('userType', 'admin', { path: '/' });
                Cookies.set('userId', data.userId, { path: '/' });
                router.push('/admin/home');
                // Logic to redirect user or save token goes here
            } else {
